...
def merge(sorted_left, sorted_right, merged_array = [])
  until sorted_left.empty? && sorted_right.empty?
    if sorted_left.empty?
      ...
      # If the left side is bigger than the right side, we will shift the first element and push it to the array
      # If the right side if bigger, we will shift that element and push it to the array until one is empty
