module TicTacToe
  ...
  class ComputerPlayer < Player
    ...
    def select_position!
      ...
      # Could make this smarter by somtimes doing corner trap offense
    ...
    def look_for_winning_or_blocking_position(opponent_marker)
      for line in LINES
        ...
        if markers[self.marker].length == 2
          ...
          log_debug "could not block on line #{line.join}"
    ...
    def corner_trap_defense_position(opponent_marker)
      ...
      # Playing in an adjacent corner could be safe, but would require more logic later on
      opponent_position = @game.board.find_index {|marker| marker == opponent_marker}
      safe_responses = {1=>[2,4], 3=>[2,6], 7=>[4,8], 9=>[6,7]}
