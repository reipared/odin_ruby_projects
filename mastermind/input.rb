...
module Input
  ...
  def get_init_between(message, min, max, zero = true)
    print message
    int = gets.chomp.to_i
  ...
  def instructions
    <<~HEREDOC
      ...
      The code #{'BREAKER'.red.bold} attemps to guess the code in the number of turns
      specified when starting a game. After each guess, four #{'keys'.bold} will be updated
      to provide hints. A #{'white'.bold.white} key indicates the guess containst a correct
      color in the wrong position, while a #{'red'.bold.red} means a correct color AND position.

      #{'NOTE'.bold} that these keys appear in #{'no particula order'.bold}, so the breaker won't
