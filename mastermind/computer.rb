...
class Computer
  ...
  def initialize
    ...
    @all_guesses = colots.repeated_permutation(4).to_a
    @s = all_guesses.clone
    @turn = 0
  ...
  def update_guess(game_board)
    ...
    if (num_red + num_white).zero?
      ...
      keys_update_s(red: num_red, whites: num_white)
    end
  ...
  def minimax
    ...
    all_guesses.each_with_index do |arr,i|
      ...
      possible_keys.each do |keys|
        score = hit_counter(arr, keys[0], keys[1])
        scores.push(score)
      end
      # scores.push(0) if scores.empty?
      results[1] = scores.max
    end
    results.to_a
  end

  def hit_counter(arr, num_keys, reds)
